...
/* The number of system clock interrupts per second.  This sets the quanta and
 * drives kernel execution.  Increasing this will improve system timeout
 * precision, but will also increase CPU utilization due to increased kernel
 * context switching.  Set this to match the needs of the system.  For example,
 * if you require a 500Hz execution schedule, this would have to be at least
 * faster than 500Hz.
 */
#define CFE_PSP_SYS_CLK_INTS_PER_SEC        1600

/* The number of aux clock interrupts per second.  This does not drive kernel
 * execution.  A good use of this would be for an interrupt driven cyclic
 * activity that is much faster than the execution of the primary tasks.  Having
 * it driven by the aux clock lets you increase the rate without increasing
 * kernel context switching and decreasing performance.
 *
 * SP0-S NOTES on the system aux clock:
 * The aux clock rate on the SP0-S is limited to specific values of:
 *   23, 47, 95, 190, ... 24414.
 * (See config.h AUX_CLK_RATE_MIN & AUX_CLK_RATE_MAX for the limits, 23 and 24414.)
 * This depends upon the single bit of the divider.  Rates requested outside of
 * valid range vai sysAuxClkRateSet() will return an error.
 *
 * SP0-S WARNING: The value of sysAuxClkRateGet() will match a valid requested
 * sysAuxClkRateSet() while the aux clock is disable.  However the aux clock
 * rate will jump to one of the above specific rates upon sysAuxClkEnable().
 * Always use the sysAuxClkRateGet() for the current value.
 */
#define CFE_PSP_AUX_CLK_INTS_PER_SEC        95

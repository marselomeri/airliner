...
#
# File:  ReadMe.txt
# Desc:  Description of the MFAB cFS application
#
#######################################################################################
...
1) MFAB_WAKEUP_MID
   ...
   it sends out its output data (MFAB_OUT_DATA_MID) before going back to the
   listening mode.
...
2) MFAB_SEND_HK_MID
   ...
   message, the application sends out its latest housekepping data (MFAB_HK_TLM_MID)
   ...
   MFAB_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the MFAB_WAKEUP_MID and MFAB_SEND_HK_MID commands over other
messages.

...
#
# File:  ReadMe.txt
# Desc:  Description of the PARAMS cFS application
#
#######################################################################################
...
1) PARAMS_WAKEUP_MID
   ...
   it sends out its output data (PARAMS_OUT_DATA_MID) before going back to the
   listening mode.
...
2) PARAMS_SEND_HK_MID

   This message is normally sent by the CFS message scheduler.  Upon receiving this
   message, the application sends out its latest housekepping data (PARAMS_HK_TLM_MID)
   ...
   PARAMS_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the PARAMS_WAKEUP_MID and PARAMS_SEND_HK_MID commands over other
messages.

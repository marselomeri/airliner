...
#
# File:  ReadMe.txt
# Desc:  Description of the PMC cFS application
#
#######################################################################################
...
1) PMC_WAKEUP_MID
   ...
   it sends out its output data (PMC_OUT_DATA_MID) before going back to the
   listening mode.
...
2) PMC_SEND_HK_MID
   ...
   message, the application sends out its latest housekepping data (PMC_HK_TLM_MID)
   ...
   PMC_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the PMC_WAKEUP_MID and PMC_SEND_HK_MID commands over other
messages.

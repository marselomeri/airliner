...
{
	strncpy(EA_AppData.ChildData.AppInterpreter, APP_PATH, OS_MAX_PATH_LEN);
	strncpy(EA_AppData.ChildData.AppSript, TEST_ARG, OS_MAX_PATH_LEN);
	...
	/* Verify results */
	UtAssert_True(Ut_CFE_EVS_GetEventQueueDepth()==1,"Event Count = 1");
	UtAssert_EventSent(EA_INF_APP_START_EID, CFE_EVS_INFORMATION, 
						"External application started", "External application started");
...
void Test_EA_Custom_TermApp_Nominal(void)
{
	...
	UtAssert_StrCmp(EA_AppData.HkTlm.LastAppRun, "Active External App", "ChildAppTaskInUse = FALSE");
}

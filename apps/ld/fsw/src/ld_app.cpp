...
boolean LD::DetectGroundContactState() {
    ...
    const boolean manual_control_idling = (ManualControlPresent()
            && CVT.ManualControlSetpointMsg.Z
    ...
    boolean vertical_movement = fabsf(CVT.VehicleLocalPositionMsg.VZ)
            > (ld_params.lndmc_z_vel_max * armingThreshFactor);

    minimal_thrust = MinimalThrust();
    altitude_lock = AltitudeLock();
    if (manual_control_idling_or_automatic && minimal_thrust

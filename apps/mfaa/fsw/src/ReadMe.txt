...
#
# File:  ReadMe.txt
# Desc:  Description of the MFAA cFS application
#
#######################################################################################
...
1) MFAA_WAKEUP_MID
   ...
   it sends out its output data (MFAA_OUT_DATA_MID) before going back to the
   listening mode.
...
2) MFAA_SEND_HK_MID
   ...
   message, the application sends out its latest housekepping data (MFAA_HK_TLM_MID)
   ...
   MFAA_SEND_HK_MID message.  It should contain ONLY essential data that the ground
...
the MFAA_WAKEUP_MID and MFAA_SEND_HK_MID commands over other
messages.

...
void CFE_PSP_DisplayTimerResolution(void)
{
	...
	printf("CFE_PSP: CLOCK_REALTIME resolution is %is %ins\n",
			(int)timeRealTime.tv_sec,
			(int)timeRealTime.tv_nsec);

	printf("CFE_PSP: CLOCK_MONOTONIC resolution is %is %ins\n",
			(int)timeMonotonic.tv_sec,
			(int)timeMonotonic.tv_nsec);
      ...
      switch( opt ) 
      {
         ...
         case 'M':
            ...
            if ((strncmp(CommandData.RunMode, "NORMAL", CFE_PSP_MODE_NAME_LENGTH ) != 0 ) &&
                (strncmp(CommandData.RunMode, "TEST", CFE_PSP_MODE_NAME_LENGTH ) != 0 ))
            {
...
void CFE_PSP_DisplayUsage(char *Name )
{
   ...
   printf("             TEST    for running in frame lock step with sims or test engines\n");
   ...
   printf("       Long form:\n");
   printf("       %s --reset PO --subtype 1 --cpuid 1 --cpuname CPU1 --scid 32 --runmode NORMAL\n",Name);
   printf(" \n");

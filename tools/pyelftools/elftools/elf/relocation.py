...
from .enums import (
    ENUM_RELOC_TYPE_i386, ENUM_RELOC_TYPE_x64, ENUM_RELOC_TYPE_MIPS, ENUM_RELOC_TYPE_ARM, ENUM_RELOC_TYPE_PPC, ENUM_RELOC_TYPE_SPARC)
...
class RelocationHandler(object):
    ...
    def _do_apply_relocation(self, stream, reloc, symtab):
        ...
        elif self.elffile.get_machine_arch() == 'SPARC':
            if not reloc.is_RELA():
                raise ELFRelocationError(
                    'Unexpected REL relocation for SPARC: %s' % reloc)
            recipe = self._RELOCATION_RECIPES_SPARC.get(reloc_type, None)
    ...
    _RELOCATION_RECIPES_SPARC = {
        ENUM_RELOC_TYPE_SPARC['R_SPARC_NONE']: _RELOCATION_RECIPE_TYPE(
            bytesize=4, has_addend=True, calc_func=_reloc_calc_identity),
        ENUM_RELOC_TYPE_SPARC['R_SPARC_32']: _RELOCATION_RECIPE_TYPE(
            bytesize=4, has_addend=True,
            calc_func=_reloc_calc_sym_plus_addend),
        ENUM_RELOC_TYPE_SPARC['R_SPARC_UA32']: _RELOCATION_RECIPE_TYPE(
            bytesize=4, has_addend=True,
            calc_func=_reloc_calc_sym_plus_addend),
    }

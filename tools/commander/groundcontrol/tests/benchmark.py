...
if __name__ =='__main__':
    ...
    """
    sockio = []
    with open('SOCKETBW.txt', 'r') as f:
        for each in f.readlines():
            sockio.append(  (each.replace("\"", "")).split(',')  )

    channels10 = []
    with open('CHANNELSBW10.txt', 'r') as f:
        for each in f.readlines():
            channels10.append(  (each.replace("\"", "")).split(',') )


    channels30 = []
    with open('CHANNELSBW30.txt', 'r') as f:
        for each in f.readlines():
    ...
    z = []
    st = int(sockio[0][num])
    st1 = int(channels10[0][num])
    st2 = int(channels30[0][num])
    for e in sockio:
        try:
            if type == 1:
                x.append(int(e[num])-st)
            else:
                x.append(int(e[num]))
        except:
            print e

...
static uint8_t frsky_buttons[JOYSTICK_NUM_MODES] = { 8, 9, 2, 0, 1, 3}; // Not accurate
...
static struct joystick_axis frsky_axes[JOYSTICK_NUM_AXIS] = {{1, 1}, {2, -1}, {0, 1}, {3, 1}};
...
int joystick_set_type(struct joystick *joystick, char *type)
{
    ...
    } else if (!strcmp(type, "frsky") || !strcmp(type, "frsky")) {
        memcpy(&joystick->buttons, frsky_buttons, sizeof(joystick->buttons));
        memcpy(&joystick->axes, frsky_axes, sizeof(joystick->axes));
    } else {

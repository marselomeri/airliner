...
{# Find correct association for required structs #}{# Check is sub message is in top level parent #}{% if airliner_msg == parent_msg[0] %}	{{ req_pb_data.airliner_msg }} {{ req_pb_data.airliner_msg }}_sub_message = inObject->{{ proto_data.fields[req_pb_data.parent_field]["airliner_name"] }};{% else %}	{{ req_pb_data.airliner_msg }} {{ req_pb_data.airliner_msg }}_sub_message = {{ parent_msg[0] }}_sub_message.{{ req_pb_data.parent_field }};{%- endif -%}{# Recur into next required message if present #}{%- do parent_msg.insert(0, req_pb_data.airliner_msg) -%}{%- do parent_pb.insert(0, req_pb_msg) -%}{%- if req_pb_data.required_pb_msgs|length > 0 -%}{{ loop(req_pb_data.required_pb_msgs.items())}}{%- endif -%}{% endfor -%}{%- set parent_msg = [airliner_msg] -%}{%- set parent_pb = [proto_data.proto_msg] -%}{%- set idx = [0] -%}
	...
	{% if var_data.airliner_type == "char" %}strcpy({{ proto_data.proto_msg[:-3] }}_msg.{{pb_var}}[{{loop.index0}}], inObject->{{var_data.airliner_name}}[{{loop.index0}}]);{% else %}{{ proto_data.proto_msg[:-3] }}_msg.{{pb_var}}[{{loop.index0}}] = inObject->{{var_data.airliner_name}}[{{loop.index0}}];{%- endif -%}{%- endfor -%}{%- else %}{% if var_data.airliner_type == "char" %}strcpy({{ proto_data.proto_msg[:-3] }}_msg.{{pb_var}}, inObject->{{var_data.airliner_name}});{% else %}{{ proto_data.proto_msg[:-3] }}_msg.{{pb_var}} = inObject->{{var_data.airliner_name}};{%- endif -%}{%- endif -%}{% endfor %}
	/* Create a stream that will write to our buffer. */
